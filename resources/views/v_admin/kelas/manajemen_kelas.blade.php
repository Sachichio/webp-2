...
<div class="container">
    ...
    <div class="title-container">
        <h1 class="title">Manajemen Kelas</h1>
    </div>

    <!-- Block 2: Table -->
    <div class="table-container">
        <div class="table-header">
            <a href="{{ route('admin.createKelas') }}" class="btn btn-primary">+ Add</a>
        ...
        <table class="table table-striped table-bordered" id="kelasTable">
            <thead>
                <tr>
                    <th onclick="sortTable(0)">Nama Kelas</th>
                    <th onclick="sortTable(1)">Jenjang</th>
                    <th onclick="sortTable(2)">Wali Kelas</th>
                    <th>Jumlah Siswa</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $kls->jenjang }}</td>
                    <td>
                        @if($kls->guru->isNotEmpty())
                            <p>{{ $kls->guru->first()->nama }}</p>
                        @else
                            <p>-</p>
                        @endif
                    </td>
                    <td>
                        {{ $kls->siswa->count() }} <!-- Menampilkan jumlah siswa yang terdaftar di kelas -->
                    </td>
                    <td>
                        {{ $kls->pelajaran->count() }} <!-- Menampilkan jumlah mata pelajaran yang terdaftar di kelas -->
                    </td>
                    <td>
                        <div class="btn-group">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.editKelas', $kls->id) }}" class="btn-secondary-edit" style="display: flex; align-items: center;">
                                <img src="{{ asset('backend/icons/edit.svg') }}" alt="Edit" style="width: 16px; height: 16px; vertical-align: middle; margin-right: 5px;">
                                Edit
                            </a>
                             <!-- Delete Button -->
                            <form action="{{ route('admin.deleteKelas', $kls->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
        ...
        <div class="pagination-controls">
            <div class="pagination-info">
                <span id="entriesInfo">Showing 1 to 5 of {{ $kelas->count() }} entries</span>
            </div>
...
<script>
    ...
    function sortTable(columnIndex) {
        ...
        const compareFunction = (a, b) => {
            ...
            // Untuk kolom lainnya (Nama Kelas dan Wali Kelas), gunakan perbandingan string
            return sortOrder[columnIndex] 
