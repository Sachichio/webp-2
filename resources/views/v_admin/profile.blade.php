            ...
            <div class="profile-wrapper">
                <div class="profile-left">
                    <div class="foto-preview-container">
                        ...
                        <img src="{{ file_exists(public_path('uploads/profile/admin/' . Auth::user()->username . '/' . Auth::user()->foto)) 
                            ? asset('uploads/profile/admin/' . Auth::user()->username . '/' . Auth::user()->foto) . '?timestamp=' . time() 
                            : asset('uploads/profile/default.jpg') }}" 
                            alt="Admin Profile Picture" class="foto-preview">
                        ...
                        <!-- Tombol Hapus Foto, hanya tampil jika foto bukan default -->
                ...
                <div class="profile-right">
                    <div class="form-group">
                        <label>Nama</label>
                        <input type="text" name="nama" value="{{ old('nama', $admin->nama) }}" class="form-control @error('nama') is-invalid @enderror">
                        @error('nama')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        <label>No. HP</label>
                        <input type="text" name="hp" value="{{ old('hp', $admin->hp) }}" class="form-control @error('hp') is-invalid @enderror">
                        @error('hp')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
<script>
    function deletePhoto() {
        if (confirm('Apakah Anda yakin ingin menghapus foto ini?')) {
            fetch("{{ route('admin.profile.destroyPhoto') }}", {
                method: 'DELETE',
                headers: {
                ...
                if (response.ok) {
                    // Perbarui tampilan foto di profile menu dan admin master
